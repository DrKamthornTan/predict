...
def plot_timeseries(rn, predicted_values=None):
    ...
    fig.update_layout(
        title=f"Trends for Record {rn} (Mouse over แต่ละจุดจะแสดงค่า)",
        xaxis_title="Year",
        yaxis_title="Value",
        height=800,
        legend=dict(
...
# Streamlit app
st.title("DHV Time-series Trend Analysis and Prediction")

# Get the unique years in the data
unique_years = df['year'].dt.year.unique()

# Allow the user to select the years they are interested in
selected_years = st.multiselect('เลือกปีที่มีผลการตรวจ', unique_years, default=unique_years)
...
record_class = st.selectbox("เลือกกลุ่มเพื่อแสดง RN ของแต่ละปี", rn_data.keys())
record_id = st.selectbox("เลือก RN", rn_data[record_class]['rns'])
